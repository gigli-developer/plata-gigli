...
export default function PrivacyToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={`relative grid h-10 w-10 place-items-center rounded-full border border-line bg-surface/70 transition-colors ${active ? "border-lime/40 text-lime" : "text-muted hover:text-fg"} ${className}`}
    >
