...
7c. **Corte nominal vs corte efectivo** (2026-09-21). El día de cierre de la tarjeta (25) NO es el día en que Galicia corta: el consumo entra al resumen cuando el **comercio lo presenta**, y eso tarda días. Agosto 2026: cierre nominal 25, el banco cortó con alertas hasta el **18-08**, y Plata (que ve el mail al instante) había metido tres consumos del 20, 23 y 24 → $13.069,04 y US$ 19,99 de más. Julio no falló por casualidad: el cierre estaba en 22-07, que era justo el corte real.
   - El desfasaje **cambia todos los meses**, así que no se arregla con un número fijo: el flujo es mirar el PDF, corregir el cierre del resumen y que los consumos sigan.
   - `reassignStatementConsumos(sb, cardId)` (db.ts) hace eso: reimputa cada consumo al **primer resumen abierto cuyo cierre no lo deje afuera**. Lo dispara el checkbox "Reacomodar los consumos" de `EditDatesModal` al guardar fechas.
   - **Solo toca resúmenes NO pagados**, en los dos sentidos: un pagado tiene el total congelado y ya reconciliado, moverle un consumo lo descuadraría contra su propio total. Las cuotas no se tocan (se imputan por `period_label`, no por `statement_id`).
   - El día del consumo se calcula en **ART**, no en UTC: uno de las 21:00 del 19 es "el 20" en UTC y ese corrimiento es justo el que decide de qué lado del cierre cae.
   - ⚠️ `reloadStatements` en `/tarjetas` refresca `fetchStatementConsumos` y limpia el caché de `movements`: sin eso la pantalla sigue mostrando el reparto viejo.
...
10. **Reglas de consumos** (`/reglas`, tabla `rules`, las aplica solo el `email-poller` a consumos nuevos). Condiciones combinables: texto (contiene/empieza/igual), horario (soporta rango nocturno 22→2), días, rango de monto. Acciones combinables: recategorizar (`category_id`, nullable), renombrar (`rename_to`), forzar moneda (`set_currency`: **convierte el monto** — la alerta de Galicia SIEMPRE reporta el equivalente en pesos, ej. TACTIQ $6.000 → USD 4,06; re-etiquetar sin convertir sería un desastre). **La cotización ya no está hardcodeada** (2026-09-21): el poller lee `fx_rate_at('mayorista', día_del_consumo, 'compra')`, que es donde liquida Visa. Contrastado contra dos resúmenes reales de la 2811: 27-06 banco 4,11 vs 4,09; 27-07 banco 4,06 vs 4,03. Con el 1455 fijo daba 4,12 las dos veces y el error crecía con cada devaluación. Los centavos que sobran son el spread de Visa (usa la cotización del día en que el comercio PRESENTA el consumo, que no conocemos). Si no hay cotización para ese día, **no convierte**: deja la fila en ARS y lo cuenta en `sinFx` — preferible una fila visiblemente en pesos que un monto en dólares silenciosamente mal. **Semántica de merge por campo**: se evalúan todas (prioridad desc, id asc) contra la descripción ORIGINAL; cada acción la define la primera regla que la tenga (pueden ser reglas distintas). El dup-check del poller NO filtra por moneda (una moneda forzada colaría duplicados).
...
- **v12 (2026-09-21)**: `openStatementId` busca el resumen por el **día del consumo**, no por la fecha en que corre el poller. Con la query `newer_than:4d`, un mail de hace tres días leído después del cierre se imputaba al resumen siguiente.
