...
6. **`get_metrics`:** `ars_liquido = ingresos_ARS − egresos_ARS_no_credito − resúmenes_pagados`. `te_deben`/`debes` usan el saldo pendiente, valuados en ARS. `usd_ars`/`usdt_ars` salen de `fx_rates` (decisión 5) — el «fijo 1455» que decía acá quedó viejo; la definición vive solo en la base (`select pg_get_functiondef('public.get_metrics()'::regprocedure)`).
   ...
   - Los tres cálculos del Cash Flow son coherentes con eso y **NO se contradicen**, aunque a primera vista lo parezca — cada uno mide algo distinto:
     - `stmtTotalArs` (bloque espejo "Pesos líquidos", ~L228): solo `total_ars`. Correcto: los dólares no salen de los pesos.
     - `stmtTotal` (tabla principal, ~L118): `total_ars + total_usd × fx_rate_ars`. También correcto, porque esa tabla valúa TODO en pesos vía `valuar`/`aggArs` — pagar con dólares sigue siendo una salida de valor.
     - `get_metrics`: descuenta cada parte de su moneda.
   - ⚠️ Antes de "arreglar" cualquiera de los tres, fijate cuál de las dos preguntas está contestando: *"¿cuántos pesos salieron?"* o *"¿cuánto valor salió?"*. Son distintas y las dos están bien.

6b. **Auditoría contra la base cruda** (`node scripts/auditar-plata.mjs`, 2026-08-13). Vuelve a calcular desde las tablas, sin importar nada de `lib/`, todo lo que devuelven las 15 herramientas del asistente: totales por período, categorías, deudas, resúmenes, saldos, patrimonio, cuotas, cotizaciones y hasta cada fila del panel. **111 comprobaciones, todas cerrando.** Correlo después de tocar `lib/db.ts`, `lib/fechas.ts` o `lib/agent/tools.ts`: encuentra en segundos las derivas que a ojo son invisibles, porque un número torcido desde el origen se ve perfectamente coherente. Deja `scripts/.verdad.json` (ignorado) para que la prueba de punta a punta verifique que el asistente DICE esos números y no otros.
...
10. **Reglas de consumos** (`/reglas`, tabla `rules`, las aplica solo el `email-poller` a consumos nuevos). Condiciones combinables: texto (contiene/empieza/igual), horario (soporta rango nocturno 22→2), días, rango de monto. Acciones combinables: recategorizar (`category_id`, nullable), renombrar (`rename_to`), forzar moneda (`set_currency`: **convierte el monto** — la alerta de Galicia SIEMPRE reporta el equivalente en pesos, ej. TACTIQ $6.000 → USD 4,11; re-etiquetar sin convertir sería un desastre). ⚠️ **La conversión usa `fx_rates` blue VENTA del día del consumo** (2026-08-03), no el `USD_ARS = 1455` que estaba hardcodeado: con el fijo, el resumen no cerraba contra el PDF del banco (jul-26: PARAMOUNT+ 3,83 vs 3,78 y Spotify 2,27 vs 2,23) y la brecha crecía a medida que el dólar se alejaba de 1455. Verificado: el blue venta reproduce los importes del banco al centavo. Se usa **venta** (lo que el banco te cobra por un consumo en dólares), no compra (que es para valuar tenencias). **El poller NO se deploya con el repo — hay que subirlo con el MCP de Supabase.** **Semántica de merge por campo**: se evalúan todas (prioridad desc, id asc) contra la descripción ORIGINAL; cada acción la define la primera regla que la tenga (pueden ser reglas distintas). El dup-check del poller NO filtra por moneda (una moneda forzada colaría duplicados).
...
- ✅ **El asistente YA sabe de deudas** (18→22/08): `deuda_pagar` cobra/salda, `plata_dividir` divide un gasto compartido (RPC `dividir_gasto`, atómica, compatible con el botón Deshacer). El bullet viejo decía que registraba "X me pagó" como ingreso suelto — murió con la tanda del 18/08.
...
- ✅ **Resumen 2026-07 de la 2811 conciliado contra el PDF (2026-08-03)**: cierra exacto en **ARS 309.505,30 / USD 37,11**. Las tres diferencias que había y para qué mirar la próxima vez:
  1. **APPLE.COM/BILL US$ 19,99 del 23-07** estaba en julio y el banco lo liquidó en agosto → se movió de `statement_id`. Es un consumo mensual que cae 2 días antes del cierre, así que **va a volver a pasar todos los meses**: el importador va en tiempo real y el banco postea los consumos internacionales con demora (decisión 7).
  2. **PARAMOUNT+ y Spotify** con centavos de más → era el `USD_ARS = 1455` del poller (ver decisión 10). Se corrigieron a los valores del banco.
  3. **Cuota de MercadoLibre** cargada en 15.000,00 vs 15.000,04 del banco.
  Método para reconciliar: los consumos sueltos salen de `transactions` con ese `statement_id`, pero **las cuotas viven aparte** en `installment_plans` — el total del banco es la suma de los dos (decisión 2). Sin sumar las cuotas la diferencia da ~$97.000 y parece un error grave cuando no lo es.
